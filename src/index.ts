...
import { CommandOptions } from './types/CommandOptions.type.js';
import { DependencyAudit } from './types/DependencyAudit.type.js';
import { parseGitHubRepoUrl } from './lib/parseGithub.js';

const program = new Command();

async function checkDependencyMaintenance(options: CommandOptions): Promise<void> {
  const packageJson: PackageJson = JSON.parse(fs.readFileSync(`${options.path}/package.json`, 'utf8'));
  ...
  const dependencyIssues: DependencyAudit[] = [];
  ...
  for (const [dep, rawVersion] of Object.entries(allDependencies)) {
    try {
      ...
      const npmInfo = JSON.parse(execSync(`npm view ${dep} --json`).toString()),
      issues = []

      // 1. Check version 1.0
      const isPreOne = await checkPreOneVersion(rawVersion);
      if (isPreOne) issues.push(isPreOne);
  ...
  if (dependencyIssues.length === 0) {
    ...
    dependencyIssues.forEach(dep => {
      const issueDetails = dep.issues.map(issue => {
        let icon = '❗';
        switch(issue.type) {
          case 'outdated': icon = '⏰'; break;
          case 'pre_1.0': icon = '🚧'; break;
          case 'archived_repo': icon = '📦'; break;
          case 'old_release': icon = '📅'; break;
        }
